...
export default function ally(allyOptions = {}) {
  ...
  return function wrapWithAlly(WrappedComponent) {
    ...
    class Ally extends Component {
      ...
      _createEventHandler(field, eventHandler = e => e.target.value) {
          const eventField = this.fields[field]
          invariant(eventField, 'Ally field not defined. Cannot use in event handler.')
          const setter = eventField.finalSetter
          return function(e) {
            invariant(eventHandler, `Ally event handler not provided. Check the onChange event for ${field}`)
            setter(eventHandler(e))
          }
        }
      ...
      configureFieldsForAlly() {
        ...
        if (!this.createEventHandler) {
          this.createEventHandler = this._createEventHandler.bind(this);
        }
